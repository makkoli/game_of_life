	...
	GameOfLife.prototype.getNextGeneration = function getNextGeneration() {
		...
		for (var i = 0; i < this.height; i++) {
			...
			for (var j = 0; j < this.width; j++) {
				...
				// Kill cell via underpopulation
				else if (neighbors < 2) {
						nextBoard[i].push(0);
					}
					// Create cell if exactly 3 neighbors and cell is dead
					else if (neighbors == 3 && this.board[i][j] == 0) {
							nextBoard[i].push(1);
						}
						// Cell lives on to next generation if between 2 and 3 neighbors
						// and ages to an older state
						else if (neighbors == 2 && this.board[i][j] != 0 || neighbors == 3 && this.board[i][j] != 0) {
								nextBoard[i].push(2);
							}
							// Else, push the state from previous board to new board
							else {
		...
		for (var i = 0; i < this.props.board.getHeight(); i++) {
			for (var j = 0; j < this.props.board.getWidth(); j++) {
				...
				if (this.props.board.getCell(j, i) == 0) {
					cellList.push(React.createElement(Cell, { status: "inactive", y: i, x: j }));
				} else if (this.props.board.getCell(j, i) == 1) {
					cellList.push(React.createElement(Cell, { status: "young", y: i, x: j }));
				} else {
					cellList.push(React.createElement(Cell, { status: "old", y: i, x: j }));
				}
			}
		}
	},
...
var Cell = React.createClass({
	...
	render: function render() {
		return React.createElement("div", { className: "cell cell-" + this.props.status, "data-y": this.props.y, "data-x": this.props.x });
	}
});
